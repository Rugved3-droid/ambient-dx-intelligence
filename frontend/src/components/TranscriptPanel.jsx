...
export default function TranscriptPanel({ transcript, intents, partialTranscript, onQuery, isProcessing }) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-3">
          ...
          if (entry.isAnswer) {
            return (
              <div key={i} className="animate-fade-in ai-answer-block">
                ...
                <div className="ai-answer-text text-sm leading-relaxed text-clinical-text/90">
                  {text}
                </div>
        ...
        {partialTranscript && (
          <div className="animate-fade-in opacity-60">
            ...
            <p className="text-sm leading-relaxed text-clinical-text/50 italic">
              {partialTranscript}
            </p>
          </div>
        )}
        ...
        {isProcessing && (
          <div className="animate-fade-in flex items-center gap-2 py-2">
            ...
            <span className="text-xs text-clinical-info">AI is thinking...</span>
          </div>
        )}
      </div>
      ...
      {/* Query Input — always visible, prominent */}
      <form onSubmit={handleQuerySubmit} className="flex-shrink-0 border-t border-clinical-border p-3 bg-[#0d1321]">
        <div className="flex items-center gap-2">
          <input
            type="text"
            value={queryText}
            onChange={(e) => setQueryText(e.target.value)}
            placeholder="Ask about this patient... e.g. 'What about sepsis?' or 'Check for HIT risk'"
